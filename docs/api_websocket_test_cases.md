...
Agent 可通过 `websocket_testcase.execute_saved` 执行单个已保存 WebSocket 用例，通过 `websocket_testcase.batch_execute` 批量执行已保存 WebSocket 用例。执行记录会写入 `trigger_source=agent`、`agent_run_id`、`agent_tool_call_id`、`trigger_tool_name`，人工前端执行继续写入 `trigger_source=manual` 且 Agent 关联字段为空。`testcase.query_project_cases` 在 `include_websocket=true` 时会额外返回 `websocket_test_case_ids`，按当前过滤结果升序列出候选 ID；同时返回 `websocket_batch_execute_input`，这是可直接传给 `websocket_testcase.batch_execute` 的推荐输入。Agent 和前端调试面板应优先使用该对象或 ID 数组，不要推断连续 ID 区间。

Agent 可通过 `websocket_testcase.create_saved` 新增已保存 WebSocket 用例，通过 `websocket_testcase.update_saved` 完整更新已保存 WebSocket 用例，也可通过 `websocket_testcase.update_assertions` / `websocket_testcase.batch_update_assertions` 只保存断言。四类写工具都复用现有 WebSocket 用例服务，要求 `case:manage` 权限，`side_effect_class=business_update`，`replay_policy=require_revalidation`，必须先进入人工审批；审批前不会写入 `websocket_test_cases`。创建工具输入为 `{"project_id":1,"case": WebSocketTestCaseCreateRequest}`，完整更新工具输入为 `{"project_id":1,"test_case_id":3,"case": WebSocketTestCaseUpdateRequest}`；单个断言更新输入为 `{"project_id":1,"test_case_id":3,"assertions":[WebSocketAssertionConfig]}`，批量断言更新输入为 `{"project_id":1,"items":[{"test_case_id":3,"assertions":[WebSocketAssertionConfig]}]}`。断言更新工具只替换 `assertions` 字段，保留 path、headers、subprotocols、messages、receive_count、connect_timeout_ms、receive_timeout_ms、extractors 和 retry_policy。审批通过并 resume 执行成功后，ToolCall 输出包含 `operation`、`project_id`、`websocket_test_case_id` 和 `websocket_test_case` 详情。

`websocket_testcase.batch_execute` 在创建任何执行记录前会先校验 `websocket_test_case_ids` 全部存在且属于当前项目。存在无效 ID 时返回 `422`，detail 形如：
...
```json
{
  ...
  "invalid_websocket_test_case_ids": [999],
  "valid_case_ids": [301, 302],
  "retry_batch_execute_input": {
    "project_id": 1,
    "environment_id": 1,
    "websocket_test_case_ids": [301, 302]
  },
...
该失败不会留下部分 `queued` WebSocket 执行记录，也不会发起 WebSocket 连接。`retry_batch_execute_input` 只是下一次工具调用的建议输入，后端不会在失败响应里自动执行它。
...
- `trigger_source`：执行来源，人工接口执行为 `manual`；Agent 通过 `websocket_testcase.execute_saved` 或 `websocket_testcase.batch_execute` 触发时为 `agent`
- `agent_run_id`、`agent_tool_call_id`、`trigger_tool_name`：仅 Agent 触发执行时写入，用于从业务执行记录追溯到 Agent Run/ToolCall
