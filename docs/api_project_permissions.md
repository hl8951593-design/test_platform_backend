...
| 方法 | `DELETE` |
| 认证 | `Authorization: Bearer <access_token>` |
| 说明 | 只有管理员或项目创建者可以删除项目；当前物理清理项目及其非 Agent 业务数据 |

删除事务显式覆盖浏览器采集、系统测试用例关系、统一执行诊断索引/证据/小时与日指标等项目资源。数据库事务成功后再尽力清理 MinIO 对象，避免对象存储故障先删附件、后因外键失败留下残缺项目；对象清理失败会记录错误日志，后续仍需 outbox/孤儿对象巡检补齐最终一致性。
