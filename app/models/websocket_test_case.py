...
class WebSocketTestCaseExecution(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_websocket_executions_project_status_created", "project_id", "status", "created_at"),
        Index("ix_websocket_executions_project_env_created", "project_id", "environment_id", "created_at"),
        Index("ix_websocket_executions_project_user_created", "project_id", "executed_by_id", "created_at"),
        Index("ix_websocket_executions_agent_tool_call_id", "agent_tool_call_id"),
    )
    ...
    trigger_source: Mapped[str] = mapped_column(String(32), nullable=False, default="manual", server_default="manual")
    agent_run_id: Mapped[str | None] = mapped_column(String(64), nullable=True)
    agent_tool_call_id: Mapped[str | None] = mapped_column(String(64), nullable=True)
    trigger_tool_name: Mapped[str | None] = mapped_column(String(128), nullable=True)
