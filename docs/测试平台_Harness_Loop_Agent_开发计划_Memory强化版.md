...
- event_seq 在 run 内递增。
- SSE 只读 EventStore；出站 data envelope 可从 `run_id/event_seq` 派生 `item_id=agent-event://{run_id}/{event_seq}`，但不得把该字段写回 `AgentEvent.payload_json`。
...
- `ai_agent_outbox.last_error` 是 dead-letter/重试排障诊断列，不是 Outbox publish response 字段；短 publish 错误保留原文，超过 512 字符时必须使用 `agent_error_message_summary_v1` preview、`agent_error_message_truncated` 标记、原始长度、hash 与 `full_error_reference=AgentOutboxPublisher.publish_pending`，不得复制完整 provider/backend 异常尾部。
...
`GET /api/v1/agents/model-health` keeps `error_message` as a string for frontend compatibility, but live probe provider/HTTP errors longer than 512 characters must use `agent_error_message_summary_v1`: preview text, `agent_error_message_truncated`, original size, hash, and `full_error_reference`, never the full provider response body or tail content.
...
- 对话型 run 失败时，`AgentRuntimeService.fail_run()` 写入的 `AgentRun.error_message` 与 `run.failed.payload.error_message` 必须保持字符串兼容但有界：短错误原样返回，超过 512 字符的 Runner/HTTP/未预期异常只返回 `agent_error_message_summary_v1` preview、`agent_error_message_truncated` 标记、原始长度、hash 与 `full_error_reference`；Conversation Smoke 和 Run Summary 只能读取同一有界字符串。
...
- 对话型 run 必须通过 AIService 写入 model.started / model.delta / model.completed，并最终写入 run.completed；主对话循环采用 planner-first：后端默认静默收完整模型输出，让模型决定普通回复或 `agent_tool_request`，解析为普通文本时只补发合并后的可见 `model.delta`，解析为工具请求时只写审计事件和 ToolCall，不把 preamble、候选分析或工具 JSON 当作 assistant 气泡展示。`model.markdown_normalized(content="", replace_content=true, normalization_reason=tool_request_stream_suppressed)` 只保留为旧路径或非主循环兼容撤回语义，正常 planner-first 工具规划不应先展示再撤回；如发生格式修复，应写入 `model.tool_request_invalid` 等审计事件。用户可见自然语言回复必须是 GitHub Flavored Markdown；后端在写入用户可见 `model.completed.content` 与 `run.completed.result.message` 前必须校准最终 Markdown，若修复了模型流式内容，应先写入 `model.markdown_normalized(replace_content=true)` 供前端替换当前气泡；工具请求型 `model.completed(requested_tool=true).content` 只允许短工具请求原文或 `agent_content_preview_summary_v1` 有界预览，不作为完整工具 JSON 来源。
...
- 同一个用户问题内允许多次 LLM 调用，必须用 Loop trace 字段把每次调用串起来：`model.started`、`model.delta`、`model.markdown_normalized`、`model.completed`、`model.stream_interrupted` 应携带 `iteration_id`、`model_call_id`、`model_response_item_id`、`loop_step`，其中 `model_response_item_id=agent-model-response://{run_id}/{model_call_id}` 用于把同一次模型调用的 delta、replacement 和 completed 归到同一个 assistant 响应项，且不得与 EventStore 事件自身 `item_id=agent-event://{run_id}/{event_seq}` 混用；工具计划、修复、最终总结和意图能力 guard 不得在事件层混成一次不可解释的模型调用；`model.tool_request_detected.reason/decision_reason` 与 `tool.*` 审计事件的 `decision_reason` 应在可得时携带，但必须保持字符串兼容且有界，短 reason 原样返回，超过 512 字符时使用 `agent_error_message_summary_v1` preview、`agent_error_message_truncated` 标记、原始长度、hash 与 `full_error_reference=AgentConversationRunner.model.tool_request_detected.reason` 或 `AgentConversationRunner.tool_trace.decision_reason`。
...
- 对话型 run 组装服务端 conversation history 时必须有预算压缩：模型 prompt 只读取同一 conversation 的已完成历史 run；长历史超过估算 token 预算后，较早轮次压成 system 摘要，最近若干轮保留截断内容，并写入 `context.history_compacted` 审计事件；该摘要不作为 assistant 气泡展示。已完成历史 run 的用户 `intent` 可以作为多轮上下文保留；历史 assistant 回复只有在对应 run 的 `assistant_visible` 未显式为 `false` 时才可回放给模型，queued/running/paused 等未完成 run 与 smoke/debug/auto-complete 等不可见结果不得进入下一轮模型上下文。
...
- 用户疑似要求保存正式场景但 ToolRegistry 尚未暴露保存工具时，AgentConversationRunner 必须通过命中 Skill 的私有 `guard_unsupported_capability` 规则处理：规则声明预检查关键词、缺失工具集合、分类 prompt、分类 JSON 字段、最终消息资源和 `completion_source`。分类 prompt 与最终回复必须来自 `scenario-composition` Skill 的私有资源文件，而不是 Runner 主 prompt 或 Python 消息常量；其中会发送给模型的 classifier prompt 必须受 `AGENT_UNSUPPORTED_CAPABILITY_CLASSIFIER_PROMPT_MAX_CHARS` 硬上限保护，超长资源以 `agent_classifier_prompt_truncated` 标记截断。只有确认需要保存且 run 仍未 terminal 时才直接说明当前只能生成草稿或 dry-run，不能假装保存，也不能重新 compose 草稿冒充保存结果；若 classifier 调用期间 Stop 已写入 `run.cancelled`，不得继续写 guard synthetic completion 或 `run.completed`。若用户明确说“不要保存/仅生成草稿”，必须继续进入 query-first 场景组合链路。
- 当模型输出的工具请求 JSON 不合法时，AgentConversationRunner 必须写入 `model.tool_request_invalid`，给模型一次格式修复机会；修复模型调用只能接收 `AGENT_REPAIR_CONTEXT_MAX_CHARS` 内的上一轮模型输出上下文，超长非法 JSON 或混合自然语言必须以 `agent_repair_context_truncated` 截断，不能把完整异常文本重新注入模型；`model.tool_request_invalid.payload.error_message`、LoopObservation `observation_json.error_message`、修复 prompt 中的错误摘要以及 `model.tool_request_repair_failed.payload.error_message` 都必须保持字符串兼容但有界，短错误原样返回，超过 512 字符时使用 `agent_error_message_summary_v1` preview、`agent_error_message_truncated` 标记、原始长度、hash 与 `full_error_reference`；`model.tool_request_invalid.content_preview`、`model.tool_request_repair_failed.content_preview` 和相关 LoopObservation `content_preview` 也必须保持字符串兼容但有界，超长模型输出使用 `agent_content_preview_summary_v1`、`agent_content_preview_truncated`、原始长度、hash 与 `full_content_reference`；修复模型返回后必须在解析或本地挽救 repaired 内容、写入 `model.tool_request_repaired/model.tool_request_repair_failed` 或创建 ToolCall 前再次检查 run terminal，若 Stop 已生效则保留 `run.cancelled`；修复成功写入 `model.tool_request_repaired` 并继续进入 ToolCall，repair 输出若混入自然语言但含单个合法工具块则以 `repair_strategy=salvaged_repair_fenced_tool_request` 记为成功修复，修复失败写入 `model.tool_request_repair_failed` 并按模型错误终止 run。
- 如果 repair 模型仍连续输出多个合法 `agent_tool_request` fenced block，Runner 不得一次创建多个 ToolCall，也不得直接失败；必须按单步工具循环只解析第一个合法工具块，写入 `model.tool_request_repaired(repair_strategy=salvaged_repair_first_tool_request_block)`，再进入 `model.tool_request_detected -> ToolCall`。后续工具由下一轮模型基于最新 ToolResult 重新规划，保证 ExecutionLedger、审批、取消检查和前端时间线保持一轮一个工具事实。
- 如果最终回复引用了不在最新 `testcase.query_project_cases` 结果中的用例 ID，Runner 必须写入 `model.final_response_reference_invalid` 并触发 `final_response_reference_repair`；若该 repair 返回合法 `agent_tool_request`，必须写入 `model.final_response_reference_tool_request_repaired` 并继续 `model.tool_request_detected -> ToolCall -> tool.result_observed -> 下一轮模型`，不得把 repair 的工具请求当作用户可见最终回答或直接 `run.completed`。只有 final summary 阶段仍使用 `final_summary_tool_request_suppressed` 安全兜底。
...
- 模型工具请求 JSON 只作为 Runner 内部规划输入和 EventStore 有界审计预览，不作为前端 assistant 气泡展示内容；`model.completed(requested_tool=true).content` 对超长内部 `agent_tool_request` 必须使用 `agent_content_preview_summary_v1`、截断标记、长度、hash 与 `full_content_reference=AgentConversationRunner.model.completed.tool_request.content`，完整结构化事实以 `model.tool_request_detected`、ToolCall Detail 和 ExecutionLedger 为准。后续模型上下文回放上一轮工具请求时也必须使用 `agent_tool_request_context_summary_v1` 有界摘要，不得复制完整 fenced JSON 或长 reason/input 尾部。最终用户可见回复以后续 `model.delta`、`model.markdown_normalized`、用户可见 `model.completed.content` 和 `run.completed.result.message` 为准，其中最终 summary 必须可直接按 Markdown 渲染。工具结果后的最终回复默认受预算约束，只输出已完成、已修复/验证、剩余阻断项和下一步；完整步骤、草稿结构和原始输出由 ToolCall 详情、summary 或报告详情承载，模型回灌消息不得承担完整长 JSON 传输职责。
...
- Outbox 发布失败后可重试。
- cancel 后不能继续调度 tool_call，也不能在外部取消已经写入 `run.cancelled` 后继续写入 `model.completed(final_summary=true)`、guard synthetic completion、`run.completed` 或 `run.failed`。`AgentConversationRunner` 必须在模型 stream、partial stream interruption 事件落库前、late tool request suppression 审计事件落库前、stream 返回后的 Markdown normalization / delta flush / `model.completed` 写入前、unsupported capability guard classifier 返回后、guard synthetic completion 前、工具请求 repair 模型返回后的解析/修复事件落库前、异常处理器写 `run.failed` 前、ToolCall 创建前、每次重新调用模型前、ToolCall 执行返回后和 final summary 前后重新读取 run terminal 状态；`AgentRuntimeService.complete_run()` / `fail_run()` 公共终态写入口也必须先刷新 terminal，禁止覆盖 `completed/failed/cancelled`；`AgentRunResumeService` 在 approval resume 执行已批准阻断 ToolCall 后也必须先检查 terminal，再决定是否启动审批后的 final summary，避免 Stop 与后台生成竞争。
...
- 对话型主循环的自然语言输出必须遵循 planner-first 安全边界：普通文本可在模型 stream 完成并完成工具请求解析后合并补发为一个 `model.delta`，不再要求首个可见 delta 早于 provider stream 结束；SSE 对 `queued/running` run 使用短轮询，对非活跃状态保持普通轮询和 heartbeat。前端仍只依赖 `model.delta.content` 追加语义，不依赖 token 粒度，也不得把缺少早期 delta 当成异常。
...
- 通用工具结果质量闭环可证明：第一次 ToolCall 返回可修复 warnings/issues/diagnostics 后，下一轮模型必须收到质量闭环规则、问题分类和来自对应 ToolSpec 私有 `tool_result_repair_guidance` 的推荐修复路径，并可再次调用合适的安全工具修复或验证；若 ToolCall 输出很大，单条模型消息必须仍在硬上限内，并以 preview/hash/reference 指向 ToolCall 完整输出；若同一 final summary 前聚合多条大工具结果，总工具结果上下文也必须受 `AGENT_TOOL_RESULT_CONTEXT_TOTAL_MAX_CHARS` 约束且不得包含截断尾部 sentinel；最终回复只把鉴权令牌等用户阻断项或已重试仍无法自动修复的剩余风险交给用户；如果同一失败签名重复出现，必须出现 `loop.observed(RC_NO_PROGRESS_PURE)` 与 `run.failed(agent_repair_no_progress)`，而不是继续第三轮同类 ToolCall。
- 模型工具请求格式轻微错误时可自动修复一次：model.tool_request_invalid -> model.tool_request_repaired -> model.tool_request_detected -> tool.result_observed -> 后续 model.delta -> run.completed；如果能从原始自然语言中的单个 fenced 工具块本地挽救，则可不额外触发 `model.started(repair_attempt=true)`，并在 `model.tool_request_repaired.repair_strategy` 标记为 `salvaged_fenced_tool_request`；触发 LLM 修复后若 repaired 内容仍混入自然语言但含单个合法工具块，也必须本地挽救并标记 `salvaged_repair_fenced_tool_request`；repair prompt 中的上一轮模型输出必须受 `AGENT_REPAIR_CONTEXT_MAX_CHARS` 硬上限保护；若 Stop 发生在 repair 模型返回后的解析窗口，必须保留 `run.cancelled`，不得再写 `model.tool_request_repaired`、创建 ToolCall 或完成 run。
- 场景组合 query-first 的 intent markers 必须覆盖“场景组合 / 测试场景组合”，并通过统一短语匹配兼容“测试场景生成”等常见换位表达；主循环工具决策由模型 planner 输出决定，Skill marker 只作为 required follow-up 和审计治理辅助防线。read_context、query cases 和 compose 之间必须保持工具规划静默收流；不得把“接下来拉取用例清单/已有场景”这类工具规划文本作为 assistant 气泡实时展示。
- 工具规划轮不会泄露完整内部 `agent_tool_request` JSON；planner-first 主循环正常会先静默收完整模型输出，工具块晚于 preamble 到达时也不应先展示临时气泡。`model.markdown_normalized(replace_content=true, normalization_reason=tool_request_stream_suppressed)` 仅作为旧路径或非主循环兼容撤回语义保留；`model.completed(requested_tool=true).content` 对长工具请求必须只保留有界预览和 hash/reference，不包含尾部 sentinel；第二轮模型 prompt 中上一轮工具请求也必须只以 `agent_tool_request_context_summary_v1` 摘要回放，不包含长 reason/input 尾部 sentinel；混合自然语言与工具请求会触发 `model.tool_request_invalid` 修复或本地挽救，repair 返回的混合自然语言 + 单工具块也必须本地挽救，query 后缺失必需 compose 会触发 `model.required_tool_missing -> model.required_tool_repaired`，失败 ToolCall 的 schema/input 错误会触发重试。
- 保存正式场景无工具时可证明 guard：`scenario-composition/SKILL.md` 的私有 `guard_unsupported_capability` 规则会先做关键词预检查和结构化语义分类，确认需要持久化且缺少 `scenario.save/create/persist` 后不调用 `scenario.compose_draft`，直接用 Skill 私有最终消息完成并说明当前无保存工具；发送给模型的私有 classifier prompt 必须有硬上限且超长资源不会把尾部 sentinel 注入模型；若 Stop 发生在 classifier 调用期间，后端必须保持 `cancelled`，不得写 guard synthetic completion 或 `run.completed`；“不要保存/仅生成草稿”的请求不得触发该 guard，必须继续 query -> compose。
- unsupported capability classifier 的 provider/HTTP 失败、非 JSON 响应或成功分类结果中的长 `reason` 只写 `agent_unsupported_capability_classification_failed` / `agent_unsupported_capability_classification_invalid_json` / `agent_unsupported_capability_classified` 日志并按 guard 结果继续路径；日志错误/响应内容/reason 短文本原样保留，超过 512 字符时必须使用 `agent_error_message_summary_v1` preview、`agent_error_message_truncated` 标记、原始长度、hash 与 `full_error_reference=AgentConversationRunner.unsupported_capability_classifier`、`AgentConversationRunner.unsupported_capability_classifier.invalid_json` 或 `AgentConversationRunner.unsupported_capability_classifier.reason`，不得把完整 provider 响应尾部或模型长 reason 写入日志。
- 审批阻断的模型工具调用可在 approve + resume 后端到端完成：approval.approved -> resume freshness check -> tool.completed -> tool.result_observed -> run.resumed -> 后续 model.delta -> run.completed；resume payload 必须返回 `executed_tool_call_ids`，且所有 terminal/noop、freshness pause、no-progress blocking 等 early return 都必须同时返回 `scheduled_tool_call_ids=[]` 与 `executed_tool_call_ids=[]` 数组字段。
- approval resume 执行已批准阻断 ToolCall 时，`AgentRunResumeService` 只能复用无 queue 行的兼容路径、`queued/blocked_approval` queue item，或 `failed(last_error_code=approval_required_before_execution)` 的历史审批阻断 queue item；如果 ToolCall 已有 queue 行但失败原因为取消、queue context mismatch、effect evidence、不可执行状态或其他非审批阻断原因，resume 必须保持 blocking，不得调用 backend、不得返回 executed id，也不得启动审批后的 final summary。
- approval resume 如果没有实际恢复进展，不得写 `run.resumed`：当 freshness check 通过，但没有已批准 ToolCall 执行成功、没有 `failed_retryable` 被重新排队，且仍有 blocking ToolCall 时，后端只能提交 `checkpoint.freshness_checked` 并返回 `resumed=false/needs_human`；前端和评测不得把这种 no-progress resume check 当作恢复成功。若本次 resume 已经重新 enqueue `failed_retryable`，这些 `scheduled_tool_call_ids` 必须从残留 blocking 计算中清除。
- Run Summary 的 `can_resume` 必须与 resume service 和 Action State 对齐：只要 run 内存在 `failed_retryable` ToolCall 且没有 pending approval/open migration block，`can_resume=true`；只要 run 仍有 pending approval 或 open migration block，`can_resume=false`，前端应优先展示/调用 `review_approvals` 或 `resolve_migration`，不得让 RunInspector 摘要按钮绕过审批或 migration block 直接调用 resume。
- 对话型 run 命中项目 Memory 时，事件流必须包含 `memory.context_injected`，`GET /api/v1/agents/memory-usage-events` 带 `run_id` 查询参数必须能看到 `usage_role=conversation_context`、`retrieval_profile=normal_plan_v1`、`active_for_policy=false` 的 usage event；构造超长 Memory 候选时，模型消息长度必须仍受 `AGENT_MEMORY_CONTEXT_MESSAGE_MAX_CHARS` 约束且不得包含截断尾部 sentinel。
- 前端发送普通对话 prompt 后，主循环不保证在模型 stream 尚未完成时收到首个 `model.delta`；planner-first 会先解析模型输出，普通文本再合并补发，工具请求则进入 ToolCall 审计链路。多个模型小片段可被后端合并成一个 `model.delta.content`，但按顺序追加后必须还原完整 assistant 回复。缺少早期 delta 时前端应保持 run pending/思考中状态，而不是判断为 SSE 故障。
- 前端 Stop 当前 run 后，后端必须保持 `cancelled` 为最终状态；即使取消发生在模型 stream 函数入口、unsupported capability guard classifier 调用期间、普通回复 stream 返回后的 Markdown normalization / delta flush 期间、工具排队后 worker 领取前、leased 工具心跳续约前、leased 工具过期恢复前、普通工具执行期间、approval resume 工具执行期间、工具结果回灌前或 final summary 流式生成/后处理期间，也不得写新的 `model.started`、继续重新采样、写 guard synthetic completion 或被后台 runner 覆盖为 completed；即使遗漏了上层 guard，`AgentRuntimeService.append_event()` 也不得在 terminal run 后追加普通 EventStore 事件或 Outbox；queued/leased 和 pre-send-intent `running_pre_effect(effect_submission_state=none,effect_boundary_crossed=false)` ToolCall 在 worker claim、worker heartbeat、执行入口或 orphan recovery 发现 run 已 terminal 时必须转为 `obsolete(agent_run_cancelled_before_tool_execution)`、queue failed，且不得发放/续约 lease、调用 backend adapter 或重新排回 queued/planned；read-only / deterministic compute 工具在执行返回后取消不得继续标为 succeeded/completed，而应转为 obsolete 并停止结果回灌；非安全 effectful 工具如果已返回输出但 run 已 terminal，必须转为 `uncertain(agent_run_cancelled_after_tool_effect)` 和 `reconcile_required_after_run_terminal`，queue failed，等待 reconcile 对账，不得继续完成 queue 或写 `tool.completed`；若 terminal run 仍残留 uncertain ToolCall，Action State 必须继续暴露 `reconcile_run`，ReconcileWorker 只能收敛 ToolCall 事实，不能重新打开或改写 run 终态；若对账需要 migration block，创建和解决 block 都不得把 terminal run 改成 `migration_blocked/running/paused`，解决后仅让 ToolCall 回到 `reconciling`；干净 `completed` run 不显示 Runbook 主入口，但 completed run 仍有 uncertain ToolCall、open migration block 或其他恢复原因时，Action State 必须启用 `open_runbook`。
- WorkerQueue claim 必须先校验 queued item 与 ToolCall 的归属一致性：`AgentWorkerQueueService.claim_next()` 在检查 terminal run、claimable status 或发放 lease 前，必须确认 queue `run_id` 等于 ToolCall `run_id`；不一致时只能把 queue 标为 `failed(tool_call_queue_context_mismatch)` 并清理 queue lease，保留 ToolCall 原事实，不得借 queue 上错误 run 的 terminal 状态或权限上下文把另一个 run 的 ToolCall 标为 obsolete/failed。
- WorkerQueue heartbeat 和 orphan recovery 也必须先校验 active queue item 与 ToolCall 的归属一致性：`AgentWorkerQueueService.heartbeat()` / `recover_orphans()` 在使用 queue `run_id` 判断 terminal、刷新 lease、恢复 queued/planned 或转 effect reconcile 前，必须确认 queue `run_id` 等于 ToolCall `run_id`；不一致时 queue 必须标为 `failed(tool_call_queue_context_mismatch)` 并清理 queue lease，若 ToolCall 仍处于 `leased/running_pre_effect` active lease 窗口，则必须转为 `uncertain`、清理 ToolCall lease，并写入 `reconcile_required_after_queue_context_mismatch`，不得用错误 run 上下文覆盖 ToolCall 或留下悬挂 lease。
- WorkerQueue heartbeat 只能续约活跃租约：`AgentWorkerQueueService.heartbeat()` 必须同时匹配 queue id、worker id 和 `status=leased`；completed/failed/blocked/queued 等非活跃队列项即使残留旧 `lease_owner`，也不得被 stale heartbeat 刷新 queue lease 或 ToolCall heartbeat/lease。
- WorkerQueue heartbeat 还只能续约可保活 ToolCall：`running_pre_effect` 可以承载 send intent/transport/backend/effect 发送窗口里的真实执行心跳；`leased` 只有在没有 effect submission evidence 时才可续约。已经 succeeded/failed/obsolete/uncertain/reconciling 或缺失时只允许 queue failed 并清理 queue lease；`leased` 已记录 send intent / transport sent / backend accepted / effect committed 或 `effect_boundary_crossed=true` 时，必须 queue failed、ToolCall 转 `uncertain` 并清理 ToolCall lease，不得刷新 ToolCall `last_heartbeat_at` 或 `lease_expires_at`。
- WorkerQueue completed/failed 终态必须清理队列自身 lease：`AgentWorkerQueueService.mark_completed()` 与 `mark_failed()` 在写入终态时必须同步清空 queue `lease_owner` 和 `lease_expires_at`，不得让成功或失败队列项继续暴露旧 worker 租约。
- ToolCall 执行器终态必须清理自身 lease：`ToolExecutor` 在 succeeded/failed/manual_intervention/obsolete/uncertain 路径中必须先保留 `policy_reason_json.execution_context.worker_id`，再清空 ToolCall `lease_owner` 和 `lease_expires_at`，不得让已完成、失败或待 reconcile 的 ToolCall 继续显示为 active worker lease。
- ToolExecutor claim 后发现 execution context 缺失时也必须清理 ToolCall lease：WorkerQueue claim 会先拒绝领取前已缺失的 AgentRun；`ToolExecutor.execute_next()` 已领取 queue/ToolCall lease 后，如果 AgentRun 在 claim 与执行上下文解析之间消失，或 run 绑定 user 缺失，必须在 backend adapter、普通工具事件和 final summary 之前拒绝执行；queue 标为 failed 并清理 queue lease，ToolCall 标为 `failed`、`execution_phase=blocked`、`error_code=run_missing/user_missing`，写入对应 `recovery_decision` 后清理 ToolCall `lease_owner/lease_expires_at`，不得留下无人接管的 active ToolCall。
- resume service 重排 `failed_retryable` 必须启动干净的新执行窗口：当 ReconcileWorker 判定 `send_intent_recorded + not_found` 可 same-key safe retry 时，`AgentRunResumeService` 在重新 enqueue 前必须把 ToolCall 改回 `planned`，同时清空上一轮 `execution_phase`、effect submission state/boundary、downstream send/transport/acceptance 证据、ToolCall lease/heartbeat 和 error 字段，只保留 `resume_retry_same_idempotency_key` 恢复决策，避免旧发送证据或旧租约污染下一次执行；如果该 ToolCall 仍残留在 `blocking_tool_call_ids_json`，本次已调度 id 必须同步从剩余 blocking 中移除，避免 safe retry 已入队但 run 继续显示 `needs_human`。
- ToolExecutor 执行入口只能执行仍可执行、queue context 归属一致且未进入发送窗口的 ToolCall：`ToolExecutor.execute_tool_call()` 在审批、权限、`tool.running`、send-intent 和 backend adapter 前，必须确认传入 queue item 的 `run_id/tool_call_id` 与 ToolCall 一致；不一致时 queue 必须标为 `failed(tool_call_queue_context_mismatch)` 并清理 queue lease，queue 所属 active ToolCall 必须转为 `uncertain`、清理 ToolCall lease，并写入 `reconcile_required_after_queue_context_mismatch`，不得调用 backend 或把错误 queue context 绑定到另一个 ToolCall。归属一致后，执行器只允许没有 effect submission evidence 的 `planned/leased` 进入执行；如果 stale queue 或 direct caller 传入已经 succeeded/failed/obsolete/uncertain/reconciling/running_pre_effect 的 ToolCall，只能把仍未终态的 queue 标为 `failed(tool_call_not_executable)` 并清理 queue lease，保留 ToolCall 原事实，不得重复调用 backend 或追加 `tool.completed`。如果 `planned/leased/running_pre_effect` 已记录 send intent / transport sent / backend accepted / effect committed / unknown，或 `effect_boundary_crossed=true`，必须把 queue 标为 `failed(tool_call_execution_after_effect_submission_started)`、ToolCall 转为 `uncertain` 并清理 ToolCall lease，不得重新写 send-intent 或触达 backend。即使 run 已 terminal，也不得用 terminal obsolete 覆盖已经 succeeded/failed/obsolete/uncertain/reconciling 或已进入发送/效果窗口的 ToolCall；只有 `planned/leased` 和 pre-send-intent `running_pre_effect(effect_submission_state=none,effect_boundary_crossed=false)` 可进入 `obsolete(agent_run_cancelled_before_tool_execution)`。
- WorkerQueue orphan recovery 只能恢复仍可安全重排的 ToolCall：若过期 queue 对应 ToolCall 仍为 `leased` 且没有 effect submission evidence，或为 `running_pre_effect` 且 `effect_submission_state` 为空/`none`、`effect_boundary_crossed=false`，recovery 可以把 queue 恢复 queued、ToolCall 恢复 planned；若 ToolCall 已经 succeeded/failed/obsolete/uncertain 或缺失，则必须把 queue 标为 failed 并清理 queue lease，不得重新排回 queued/planned；若 `leased/running_pre_effect` 已记录 send intent / transport sent / backend accepted / effect committed，或 `effect_boundary_crossed=true`，必须把 queue 标为 `failed(tool_call_orphaned_after_effect_submission_started)`，ToolCall 转为 `uncertain` 并清理 ToolCall lease，保留 effect submission 证据交给 ReconcileWorker。
...
- claim 必须同时尊重 ExecutionLedger：只有 queue 指向真实 ToolCall、queue `run_id` 与 ToolCall `run_id` 一致、对应 AgentRun 仍存在且 ToolCall 仍为 `planned` 时才可以被领取为 `leased`；queue 指向不存在的 ToolCall 时必须 `failed(tool_call_missing)` 并清理 queue lease，不得发放 active lease；对应 AgentRun 已缺失时必须 `failed(run_missing)`，把仍为 planned 的 ToolCall 写为 `failed/execution_phase=blocked`、记录 `run_context_missing_before_execution` 并清理 ToolCall lease，不得先发放 active lease 再等待 executor 兜底；`uncertain/reconciling` 必须阻断并要求 reconcile，同时清理 queue 与 ToolCall 上残留的 active lease 字段；其他非 claimable 状态只能让陈旧 queue failed，不得把 `obsolete/succeeded/failed/failed_retryable/needs_migration/manual_intervention/leased/running_pre_effect` 复活成 leased。heartbeat/orphan recovery 等 active lease 路径同样必须校验 queue/run/call 归属；不一致时 queue failed，active ToolCall 转 `uncertain` 并交给 reconcile。
...
- lease 过期后只允许 pre-send-intent 安全重排；post-send-intent 必须进入 uncertain/reconcile。
...
```text
1. 扫描 active 或 terminal run 内的 uncertain / reconciling tool_call
...
- `uncertain/reconciling` ToolCall 不允许被 WorkerQueue 直接执行；若误入执行队列，必须阻断执行并返回 `tool_call_uncertain_reconcile_required`，要求先走 reconcile，同时清理 queue 与 ToolCall 的旧 lease 字段，避免待 reconcile 状态继续表现为 worker active lease。
...
Checkpoint context compaction reference 是 Freshness Gate 的恢复审计元数据。Runner 在写入 `context.history_compacted` 后，必须把当前 checkpoint 的 `context_compaction_object_key` 更新为对应 EventStore 引用；Freshness Gate 输出该引用、事件 seq/type 和可用状态。正常可用时不改变 `result/action`；如果 checkpoint 指向的 compaction event 已丢失或引用格式不合法，则不能从该 checkpoint 继续，必须重新从最新安全状态 replan。
...
`behavior_evaluation_latest_report_fields_source` fixes the latest-report summary field list to `scripts.agent_behavior_evaluation.LATEST_REPORT_SUMMARY_FIELDS`, and `behavior_evaluation_suite_available.details.latest_report_fields` must expose that same ordered whitelist so docs, completion audit and the evaluation script cannot drift.

Behavior evaluation JSON result rows must treat `assistant_message` as a bounded report preview, not as the full assistant transcript. `run_case()` may use full `AgentRunSummary.assistant_message` for `evaluate_case()` semantic assertions, but the report row must write `agent_behavior_eval_assistant_message_preview_v1` preview text plus `assistant_message_length`, `assistant_message_truncated`, and `full_assistant_message_reference=AgentRunSummary.assistant_message`; full assistant content remains behind Run Summary/Transcript permission boundaries.

Behavior evaluation error text must also be treated as bounded report preview. ToolCall Detail fetch errors, SSE high-cursor replay errors, and single-case exceptions captured by `main()` must be written through `agent_behavior_eval_error_summary_v1` with fixed-length `error`/`fetch_error` preview text, truncation state, original size, hash, and `full_*_error_reference`; long HTTP bodies, tracebacks, provider response bodies, or tail content must not be copied in full to JSON, Markdown, or progress artifacts.

The behavior evaluation HTTP client must apply the same preview boundary before raising runtime errors. `ApiClient.request_json()` and `request_sse_text()` must format HTTPError/URLError through `agent_behavior_eval_error_summary_v1`, including preview text, truncation state, original size, hash, and `full_error_reference`, instead of embedding complete HTTP bodies or provider response bodies in exception strings.

`behavior_evaluation_suite_available.details.latest_report.duplicate_case_ids` must list any case id that appears more than once in the latest historical report. `current_case_set_complete` must be false unless `missing_case_ids`, `extra_case_ids`, and `duplicate_case_ids` are all empty. When `duplicate_case_ids` is non-empty, `model_call_trace_complete`, `tool_diagnostic_chain_complete`, and `sse_high_cursor_replay_complete` must also be false even if their corresponding `missing_*_case_ids` arrays are empty.

`behavior_evaluation_suite_available.details.latest_report.schema_matches_current` is a shared prerequisite for `model_call_trace_complete`, `tool_diagnostic_chain_complete`, and `sse_high_cursor_replay_complete`. Legacy or missing-schema reports may still expose case and diagnostic coverage arrays for human reference, but these three diagnostic complete fields must be false when `schema_matches_current=false`.

`behavior_evaluation_suite_available.details.latest_report.summary_counts_match_results` must compare `summary.case_count`, `summary.passed_count`, and `summary.failed_count` with counts derived from `results`; missing, non-integer, mismatched summary counts, or non-boolean `results[].evaluation.passed` values must return false without exposing raw result payloads.

`behavior_evaluation_suite_available.details.latest_report.summary_average_score_matches_results` must compare `summary.average_score` with the average derived from every `results[].evaluation.score` using the report generator rule `round(sum(scores) / max(case_count, 1), 1)` when all scores are finite numeric values; missing scores, non-numeric scores, non-finite scores (`NaN`, `Infinity`, `-Infinity`), non-object result rows, or mismatched score fields must return false without exposing raw result payloads.

`behavior_evaluation_suite_available.details.latest_report.invalid_evaluation_case_ids` must list case ids whose latest historical report row has non-boolean `evaluation.passed` or non-finite-numeric `evaluation.score`; it must expose only case ids, not raw evaluation payloads.

The behavior evaluation report generator must remain audit-preserving when a single case returns malformed evaluation data: `report_summary_from_results()` only counts a case as passed when `evaluation.passed is True` and `evaluation.score` is finite numeric, treats malformed rows as failed, uses 0 for invalid scores including `NaN/Infinity` in the generated average, and keeps the original result row so latest-report summary can surface `invalid_evaluation_case_ids`.

Behavior evaluation JSON artifacts must remain standards-compliant even when a partial result contains non-finite floats: `write_json()` must recursively replace `NaN`, `Infinity`, and `-Infinity` with `<non-finite-number:nan|inf|-inf>` string sentinels at the artifact boundary, then serialize with `allow_nan=false`. Summary derivation must still use strict `evaluation_score_value()`, and the sentinel score must keep the case visible through `invalid_evaluation_case_ids` rather than producing a raw non-standard JSON constant.

Latest-report readers must reject historical artifacts that still contain raw non-standard JSON constants: `latest_report_summary()` must use `parse_constant=reject_non_standard_json_constant`, return the stable unavailable summary, and expose `error=NonStandardJsonConstantError` for raw `NaN`, `Infinity`, or `-Infinity` rather than accepting Python's permissive parse result.

Markdown companion and progress logs must render the same sentinel values: `markdown_report()` must sanitize its payload with `json_safe_value()` before rendering, and `[done]` progress records must use the same safe value for `score=`. A non-finite result score must therefore appear as `<non-finite-number:nan|inf|-inf>` in JSON, Markdown, and progress artifacts, never as raw `nan`, `inf`, or `-inf`.

Progress logging and Markdown rendering must use the same resilient evaluation boundary: missing or malformed `evaluation.score`, `evaluation.passed`, `evaluation.passes`, or `evaluation.issues` must not append a duplicate error result or abort JSON/Markdown artifact generation; Markdown may render `<missing>` placeholders while latest-report summary exposes invalid/summary-integrity status.

If one `run_case()` raises, the evaluator must still emit an audit-preserving failure row instead of losing the full report: the row must carry `status=error`, zero/empty timing and event counters, `sse_high_cursor_replay.error`, empty tool/trace lists, `evaluation.passed=false`, and `score=0`, so JSON/Markdown artifacts and latest-report summary remain available with counts matching the result rows.

Markdown companion rendering must be resilient without rewriting the JSON artifact: if a result row is missing runtime, SSE replay, tool list, assistant snippet, or ToolCall fields, `markdown_report()` must render `<missing>` or empty collections and leave the original JSON row unchanged for latest-report integrity checks.

Evaluation helpers must also be resilient before report generation: `evaluate_case()` and `evaluate_common()` must convert missing runtime, assistant message, tool list, ToolCall list, or SSE replay fields into failed issues instead of raising KeyError, so a partial result row remains auditable through JSON/Markdown artifacts.

`behavior_evaluation_suite_available.details.latest_report.tool_diagnostic_chain_case_ids` must only include cases where every successfully fetched ToolCall has a `diagnostic_chain` object with boolean `execution_context_present=true`, non-empty string `execution_context_hash`, boolean `dispatch_trace_present=true`, and non-empty string `dispatch_trace_hash`; non-boolean present flags or non-string hash values must remain in `missing_tool_diagnostic_chain_case_ids`.

`behavior_evaluation_assertion_coverage_source`, `behavior_evaluation_undeclared_case_assertions_source`, `behavior_evaluation_uncovered_assertions_source` and `behavior_evaluation_runbook_source` likewise fix the assertion coverage map, undeclared assertion metadata, uncovered assertion metadata and safe runbook metadata to script helpers, and completion audit `derived_from` must expose these sources. `behavior_evaluation_suite_available.details.assertion_coverage` only includes declared assertions from `ASSERTIONS`; if any `EvalCase.assertion_ids` entry is missing from `ASSERTIONS`, `details.undeclared_case_assertions` must expose assertion -> case ids; if any declared assertion has no covering case, `details.uncovered_assertion_ids` must list it. Either condition makes `details.assertion_metadata_complete` false, and the behavior-evaluation check must move to attention instead of silently treating the suite as fully aligned.

`behavior_evaluation_suite_available.details.latest_report.model_call_trace_case_ids` must only include cases whose latest historical report result passes `scripts.agent_behavior_evaluation.result_has_model_call_trace`: non-empty `model_call_trace[]`, every item with `model_call_id`, `loop_step`, and boolean `started_event_seen=true`, and positive-integer `model_call_count` matching trace length when that count is present. Non-empty but incomplete trace summaries, including non-boolean `started_event_seen` values or non-positive/non-integer `model_call_count`, must remain in `missing_model_call_trace_case_ids`.

`behavior_evaluation_suite_available.details.latest_report.sse_high_cursor_replay_case_ids` must only include cases whose latest historical report result passes `scripts.agent_behavior_evaluation.result_has_sse_high_cursor_replay`: `sse_high_cursor_replay` must be an object without `error`, `event_count` and `non_heartbeat_event_count` must be positive integers, non-heartbeat count must not exceed total event count, and `heartbeat_only` must be false. Error summaries, missing counts or inconsistent counts must remain in `missing_sse_high_cursor_replay_case_ids`.

`GET /api/v1/agents/backend-completion-audit` is the project-scoped backend-owned Agent feature completion audit. With `project_id`, any project member can read it after project permission validation; without `project_id`, it is admin-only global audit. It aggregates the launch audit and fixed backend completion checks for conversation streaming, server-side history, tool loop, approval resume, Memory injection, frontend contract surface, observability/release-gate inputs, documentation sync, live E2E diagnostic availability, and the multi-case behavior evaluation suite. It must not run a live provider probe and must not expose `DEEPSEEK_API_KEY`. `runtime_contracts` must declare the `AgentToolCall.policy_reason_json.execution_context` source, the `AgentToolCall.policy_reason_json.dispatch_trace` source, and the `AgentRunbookRecommendation.details.execution_context` / `AgentRunbookRecommendation.details.dispatch_trace` whitelist summary fields; `diagnostics` must include ToolCall Detail, Runbook diagnosis, `scripts/agent_behavior_evaluation.py`, and `reports/woagent_behavior_eval_*.json|md` entrypoints so completion audit can lead maintainers to full execution diagnostics and behavior-eval artifacts. `behavior_evaluation_suite_available` must derive `case_ids` and `case_count` from `scripts.agent_behavior_evaluation.CASES` (currently T01-T08), derive `assertions` from `scripts.agent_behavior_evaluation.ASSERTIONS`, derive declared-only `assertion_coverage` and undeclared assertion metadata from each `EvalCase.assertion_ids`, derive safe `runbook` metadata from `scripts.agent_behavior_evaluation.behavior_evaluation_runbook()`, derive historical latest-report metadata from `scripts.agent_behavior_evaluation.latest_report_summary()`, and at least assert general answers do not call tools, conversation context does not create objects, project context uses tools, query-first order, tool-result repair loops, unsupported save boundaries, dataset parameterization, domain boundaries, ToolCall diagnostic chains, model-call trace, and SSE high-cursor replay. Report schema v2 must expose ToolCall diagnostic chains for every successfully fetched ToolCall in JSON as whitelist fields and render the same safe hash/router/runtime/backend handler summary in Markdown for human review, without copying full `policy_reason_json` or raw payload; `tool_calls[].input_json_redacted` is a backwards-compatible field name whose value must be `agent_behavior_eval_tool_input_summary_v1`, containing input keys, boolean field index, bounded preview, truncation flag, size, hash, and `full_input_reference=AgentToolCallRead.input_json_redacted`, so dataset assertions can read safe flags without copying full tool input. If any successfully fetched ToolCall in a covered case lacks the diagnostic chain, latest report must mark that case missing ToolCall diagnostic-chain evidence. The runbook must expose the local command, required env names, optional env defaults, and report schema version, but must not expose `AGENT_EVAL_PASSWORD` value. The latest report summary is historical and whitelist-only: it may expose report path, derived Markdown path, Markdown availability, JSON/Markdown artifact-pair completion, generated time, schema version, expected schema version, schema match status, summary counters, passed/failed case ids, reported/expected/missing/extra case ids, `current_case_set_complete`, model-call trace coverage fields, ToolCall diagnostic-chain coverage fields, and SSE high-cursor replay coverage fields; when no usable report exists it must still return stable artifact, `report_schema_version=null`, expected schema, `schema_matches_current=false`, expected/missing/current case set, model trace, tool diagnostic chain, and SSE replay fields with `available=false`; it must not expose login user, assistant transcript, tool payload, SSE event preview, Markdown body or secrets and must not decide `complete/status`. `derived_from` must expose `behavior_evaluation_cases=scripts.agent_behavior_evaluation.CASES`, `behavior_evaluation_assertions=scripts.agent_behavior_evaluation.ASSERTIONS`, `behavior_evaluation_assertion_coverage=scripts.agent_behavior_evaluation.assertion_coverage`, `behavior_evaluation_undeclared_case_assertions=scripts.agent_behavior_evaluation.undeclared_case_assertions`, and `behavior_evaluation_latest_report=scripts.agent_behavior_evaluation.latest_report_summary`. `complete=true` only claims the backend repository scope; frontend delivery remains external and L3 production rollout remains controlled by release gates.
...
Run 级 Runbook 诊断必须从当前 run 事实表输出可执行 recommendation，而不是只返回 catalog：至少覆盖 uncertain/reconciling ToolCall、migration block、pending/stale approval、checkpoint stale、backend capability degradation、LoopObservation 缺失 decision ContextBuild、AgentConversationRunner 写入的 runtime repair/stop LoopObservation、RootCause rule missing、Memory EvidenceRef governance violation，以及当前 release gate tool matrix violation。对于 `tool_call_uncertain` 与 `backend_capability_degraded`，recommendation 的 `details.execution_context` 必须附带 ToolCall execution context 的白名单摘要，用于展示执行 hash、worker、运行时快照、效果状态、后端能力和恢复动作；`details.dispatch_trace` 必须附带 ToolCall dispatch trace 的白名单摘要，用于展示 router/runtime/backend handler、schema/manifest hash 和调度状态；不得把原始 input/output/evidence/error message 或完整业务 payload 复制到 Runbook。对于 terminal run 上的 open migration block，`migration_blocked` recommendation 必须使用 `reason=open_migration_block_on_terminal_run`，并在 `details` 中输出 `run_status`、`run_terminal=true`、`resolve_preserves_terminal_run=true`、`post_resolve_next_action=reconcile_run` 与 `tool_call_status_after_resolve=reconciling`，让恢复面板明确 resolve block 只让 ToolCall 回到 reconcile，不会把 terminal run 恢复为 running/paused；`POST /api/v1/agents/runs/{run_id}/migration-blocks/{block_id}/resolve` 返回的 `checkpoint_freshness` 在同一 terminal 场景也必须输出 `terminal_run_preserved`、`terminal_run_status`、`resolve_preserves_terminal_run`、`post_resolve_next_action` 与 `tool_call_status_after_resolve`，并按 ToolCall 当前状态计算 `post_resolve_next_action`，让直接调用方和 Runbook 面板保持一致且不重复提示过期恢复动作。对于 `approval_conflict_event_seen` 与 `memory_bypassed_evidence_ref_event_seen`，recommendation 的 `details` 必须使用 `runbook_event_payload_summary_v1`，只输出 event id/seq、payload keys、固定长度 preview、截断状态、大小、hash 与 `full_payload_reference=AgentEvent.payload_json`，不得复制完整 `AgentEvent.payload_json`。`GET /api/v1/agents/runs/{run_id}/runbook` 必须先按 run 所属项目校验访问权限；项目成员和 admin 可读取，项目外用户必须 403，避免 Runbook 诊断泄露跨项目运行状态。
...
Run Summary 的 `can_resume` 不是单纯的 status 派生字段；当存在 `failed_retryable` ToolCall 且没有 pending approval/open migration block 时必须返回 `true`，与 `retryable_tool_calls` 和 `resume_run.enabled=true(reason=resume_candidate_ready)` 保持一致；当 run 状态仍为 `migration_blocked` 但没有 open migration block 时，Action State 也必须把 `resume_run` 作为可用主操作，让 resume/freshness 继续收敛残留状态；当 `pending_approval_count > 0` 或 `open_migration_block_count > 0` 时必须返回 `false`，与 `review_approvals`、`resume_run.enabled=false(reason=pending_approvals_need_review|open_migration_blocks)` 和 `409 run_migration_blocked` 保持一致。
...
`GET /api/v1/agents/runs/{run_id}/actions` 返回 Codex 式右侧操作区所需的只读 action state。后端必须从 Run、ToolCall、Approval、MigrationBlock 等事实表聚合可操作入口，输出固定顺序 action 列表：`view_summary`、`stream_events`、`cancel_run`、`review_approvals`、`resume_run`、`reconcile_run`、`resolve_migration`、`open_runbook`。前端只根据 `enabled`、`reason`、`resource_ids` 和 `primary_action_ids` 渲染按钮，不再自行推断 pending approval、uncertain tool 或 migration block 的优先级；`primary_action_ids` 必须独立于固定 action 列表，按 `review_approvals`、`resolve_migration`、`reconcile_run`、`resume_run`、`open_runbook`、`cancel_run` 的后端优先级过滤 enabled action；action 级 `resource_ids` 必须稳定排序和去重，同一 ToolCall 即使同时来自 blocking 与 `failed_retryable` 也只输出一次，来源解释保留在 `details` 分类字段；ToolCall 资源按 `step_index`、`attempt_index`、内部 id 排序，Approval/MigrationBlock 资源按 `created_at`、内部 id 排序。terminal run 存在 open migration block 时，`resolve_migration.details` 必须输出 `run_status`、`run_terminal=true`、`resolve_preserves_terminal_run=true`、`post_resolve_next_action=reconcile_run` 与 `tool_call_status_after_resolve=reconciling`，使右侧操作区在点击 resolve 前就能提示该操作不会恢复 terminal run。
...
`GET /api/v1/agents/conversations/{conversation_id}/transcript` 返回服务端可恢复的 Codex 式 conversation transcript，按 run 创建时间升序返回每一轮的 `AgentRunSummaryRead`，并用 `context_compactions` 暴露同一 conversation 内 `context.history_compacted` 的 redaction-safe 索引，用于前端刷新后恢复 user prompt、assistant 最终回复、运行状态、右侧计数 badge 和 compaction 审计状态。该接口必须带 `project_id` 并先校验项目访问权限；项目外用户不得通过 conversation_id 枚举历史。
...
`GET /api/v1/agents/conversations/{conversation_id}/export` 返回可下载/可调试的 Codex 式 conversation export 包。它必须复用 transcript 的项目权限校验、run 顺序和 `context_compactions` 索引，并按 run_id 分组输出 EventStore、ToolCall、Approval、MigrationBlock 派生事实；导出包只包含后端 redacted 字段，不暴露未脱敏模型 prompt、密钥或原始大对象。
...
`AgentToolCallRead.item_id` 由 ToolCall 事实派生为 `agent-tool-call://{run_id}/{tool_call_id}`，供 ToolCall Detail、审批响应、conversation export 和前端 timeline/debug item 使用；它不新增数据库列，也不写入 EventStore payload。

`AgentReconcileAttemptRead.item_id` 由对账尝试事实派生为 `agent-reconcile-attempt://{tool_call_id}/{attempt_seq}`，供 ToolCall Detail 中的 `recent_reconcile_attempts[]` 定位单次 reconcile/backoff 诊断项；它不新增数据库列，也不替代父级 `tool_call_id` 或 run-scoped ToolCall item。
...
`policy_reason_json.dispatch_trace` 必须保留 ToolExecutor 在工具进入后端 routing/runtime 后写入的调度 trace：dispatch trace version、tool/run/runtime snapshot、tool name/version、schema/manifest hash、router/runtime 名称、backend handler、backend contract、resolved side effect/replay policy、最终 status/effect submission state 和 `dispatch_trace_hash`。ToolCall Detail 和评测可以展示该摘要解释 ToolSpec/Router/Runtime 如何分派到具体后端 handler；不得写入原始 input/output/evidence 或未脱敏业务 payload。若 effect 已提交但 EventStore 写入 `tool.effect_committed` 或 `tool.completed` 失败，ToolCall 进入 `uncertain(eventstore_write_failed_after_effect)` 后必须重新生成 dispatch trace，使 `status=uncertain`、`effect_submission_state=effect_committed` 和 `dispatch_trace_hash` 与最终恢复状态一致。
