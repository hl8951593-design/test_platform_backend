...
当前 capabilities 返回 49 个 ToolSpec。相对原有 29 个工具，平台业务工具包括：`execution.query_records/read_detail/diagnose`；`plan.query_project_plans/create_saved/update_saved/set_enabled/execute_saved/query_runs/read_run`；`flow.query_project_flows/validate_graph/create_saved/update_saved/execute_saved`；`defect.query_project_defects/create_saved/update_saved/transition_status`；以及本轮新增的只读诊断工具 `agent.run.read_summary`。这是对 `tools[]` 的加法扩展，没有删除或重命名旧工具。前端不得写死工具数量；应按 `name`、`item_id`、`schema_hash` 和 `manifest_hash` 渲染当前 capabilities/runtime snapshot。
...
模型可见目录仍由 Skill Planner 与 Capability Resolver 按本轮意图裁剪：执行失败分析加载 `execution-diagnosis`；测试计划管理加载 `test-plan-management`；可视化 Flow 管理加载 `visual-flow-design`；缺陷查询和流转加载 `defect-triage`；Agent 运行故障追踪可加载 `agent-runtime-operations` 并使用 `agent.run.read_summary`。全局 capabilities 能看到 49 个工具，不表示单轮模型上下文会收到全部工具，也不改变 ToolRuntime 的权限、审批、schema 和引用预检边界。`agent.run.read_summary` 仅返回同项目 Run 的有界、脱敏状态、诊断事件和紧凑 ToolCall 摘要；跨项目 Run 按不存在处理，不返回原始 prompt、完整工具输入或完整工具输出。

LLM 规划返回的 `effect_scope` 是模型请求值，不再作为权限事实直接执行。后端会根据本轮 ToolSpec 的 `side_effect` 计算 `required_effect_scope`，再取二者中更严格的值作为最终 `effect_scope`；例如 `defect.create_saved` 即使被模型描述为 `execute` 或 `draft`，也会规范化为 `persist`，而不是在 ToolCall 创建前以 `planner_effect_scope_exceeded` 失败。Capability Plan 审计字段同时保留 `model_requested_effect_scope`、`required_effect_scope` 和 `effect_scope_normalized`。该规范化不会放宽安全边界：未知副作用、缺权限、计划外工具、项目隔离、对象引用、审批和 ToolRuntime preflight 仍然 fail-closed；所有持久化工具仍必须进入 pending approval，审批通过后才允许落库。
...
后端在当前 run 调用模型前会按预算组装同一 conversation 的已完成历史消息，并额外注入一条“同一会话工作上下文”system 消息。`conversation_working_context_v2` 继续提供结构化 `recent_turns`、`current_artifact_candidates`、`current_intent_is_deictic_followup` 和回指解析规则，并新增有界、脱敏的 `recent_run_states[]`。已完成 Run 的可见 assistant 回复仍进入自然语言历史；`failed/cancelled/paused/needs_human` 等终态或阻断态 Run 不回放 assistant 文本，但其 `run_id/status/error_code/error_message_preview/last_event/tool_calls` 等最小事实可进入 `recent_run_states[]`，让下一轮模型知道上一轮是在规划、工具或审批哪一层失败，避免把 Runtime 故障误诊为业务执行失败。该工作上下文只进入模型输入和技能选择，不作为前端 transcript/export 的新业务 payload；前端仍以 run summary、events、ToolCall 和 approval 作为展示事实源。Unsupported capability guard 只能用显式领域 subject 预拦截，不能仅凭回指词在模型读取会话工作上下文前终止 run。
