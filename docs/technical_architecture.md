...
模型初始工具提示是 Plan 阶段的精简协议，而不是完整 manifest 出口：`_conversation_system_prompt()` 从 `ToolRegistry.list_specs()` 派生 `approval_required,input_schema,name,side_effect_class,summary` 五个字段，并用 `json.dumps(sort_keys=True,separators=(",", ":"))` 输出稳定 JSON，便于 prompt cache 复用。完整 manifest 字段仍由 capabilities/runtime snapshot/ToolCall 诊断承载，后端私有 handler、前置工具和修复 guidance 只供 Runtime/Runner/Policy 内部读取。所有模型可见顶层 `environment_id` schema，以及保存类工具的嵌套 `case.environment_id` / `case.environment_ids` schema，都必须写明 `Use only ids from project.read_context.object_reference_manifest.environment_ids`，使模型在规划阶段先读取环境事实快照；执行器的 environment `ObjectReferenceGuardRule` 仍负责副作用边界前的硬校验。Harness 文档用 `Required Agent initial tool prompt contract` 机器契约锁住模型可见工具协议，避免模型规划入口与 registry、snapshot 或前端契约分叉。
...
final summary 阶段的 suppression 是防重复副作用边界，不是默认失败判定。审批恢复或迭代上限后的最终总结如果再次输出 `agent_tool_request`，Runner 仍写入不可见的 `model.completed(requested_tool=true, assistant_visible=false, final_summary_tool_request_suppressed=true)`，并以 `run.completed.result.final_summary_tool_request_suppressed=true` 结束；但 `run.completed.result.message` 必须根据已回灌 ToolCall 状态生成：当本轮 `tool_calls[]` 全部为 `succeeded` 时，用户可见消息应说明工具已成功执行且重复工具请求已被阻止；只有存在失败/阻断工具时，才提示查看 ToolCall 失败详情和修复输入。这避免“保存已成功，但最终提示失败详情”的状态漂移。
...
Agent 工具结果过长不再只依赖模型上下文截断兜底：`testcase.query_project_cases` 默认 `detail_level=summary`，并提供 `detail_level=summary|assertions|selected|full|execution_ready` 与按 `test_case_ids/websocket_test_case_ids` 精确回查，`scenario.query_project_scenarios` 提供 `detail_level=summary|full`。查询工具返回 `*_result_policy.detail_fetch_hint`，用例查询还返回 `case_result_policy.large_task_strategy(strategy_id=summary_then_selected_detail_then_execution_ready_action)`：先查摘要清单做分析，再按显式 ID 查询 selected/assertions 详情，真正批量执行或批量保存前必须重新查询 `execution_ready`，并复制返回的 `*_batch_execute_input.case_snapshot_id` 与 `object_references`。`ToolResultPolicy` 对用例查询构造专用 compact model view，完整保留 `case_id_manifest`、`case_display_rows`、`case_status_summary`、`case_attention_rows`、`object_reference_manifest` 与批量输入；headers/body/query_params 等大字段和 selected/detail 明细只进入安全裁剪视图。若模型仍需要完整红acted输出，可调用只读 `tool_result.read_full` 读取对应 `ToolCall.output_json_redacted`。Runtime 仍保留 ToolResultPolicy 和聚合上下文硬上限作为最后防线，但完整事实的可达性由查询契约和 full-result 读取工具共同保证，避免模型只能看见被截断的前几个对象后猜 ID、猜名称或猜断言。
...
同一会话工作上下文不再只依赖 assistant 文本推断可复用产物。Runner 在构造模型上下文时会查询同 project、同 user、同 conversation、当前 run 之前的成功 ToolCall ledger，生成 `conversation_tool_artifact_manifest_v1` 轻量索引；每个 item 只包含 `artifact_id`、`artifact_type`、`tool_call_id`、`tool_name`、`run_id`、`conversation_id`、`output_hash`、`output_path`、`artifact_summary` 和 `tool_result.read_full` 读取入口，不把完整 `output_json_redacted` 注入 prompt。`scenario.compose_draft` 会被识别为 `scenario_draft`，`output_path=draft.scenario`，后续“保存刚才的测试场景”应优先按该 manifest 读取完整草稿；中间用户切到测试用例、断言或普通问答本身不会让该 artifact 失效，只有对象删除、最新事实快照变化、环境/用例结构变化等真实外部事实才触发重新 query 或重新 compose。

场景草稿保存 follow-up 不依赖模型从可见摘要重构长 JSON。`scenario.compose_draft` 的完整 `draft.scenario` 以 ToolCall ledger 输出作为 authoritative artifact；当同一会话后续请求为“保存该场景/刚才的场景/保存「某流程」/保存后执行”等保存 follow-up 时，`AgentConversationRunner._normalize_tool_input()` 会在创建 `scenario.create_saved` / `scenario.update_saved` ToolCall 前读取同会话成功的 `scenario.compose_draft`。如果当前 intent 明确包含草稿名称，Runner 优先选择名称出现在 intent 中的草稿；否则回退最近成功草稿。随后用完整 `draft.scenario` 替换模型提交的退化 `scenario`，并在 ToolCall input 中记录 `scenario_draft_source`。这让场景自己的 `before_actions`、`after_actions`、`_scenario_context.extractions`、`_scenario_context.bindings` 和 downstream `{{variable}}` 请求绑定继续由后端 artifact 承载，避免跨轮历史只保留 assistant 摘要时把场景退化成测试用例列表。
