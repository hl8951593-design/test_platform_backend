...
同一会话“保存该场景/保存刚才的场景/保存「某流程」/保存后执行”属于草稿 artifact follow-up。后端会在 ToolCall 创建前优先复用成功 `scenario.compose_draft` 的完整 `draft.scenario`，避免模型从 assistant 可见摘要重构 JSON；如果当前 intent 明确包含草稿名称，优先复用名称命中的草稿，否则回退最近成功草稿。被复用的保存 ToolCall input 会包含 `scenario_draft_source={source,run_id,tool_call_id}`，供 ToolCall Detail 展示来源。该字段是诊断元数据，不是业务 `ScenarioCreateRequest` 的一部分。
...
- 当 Summary 的 `run.result_json.final_summary_tool_request_suppressed=true` 时，表示 final summary 阶段模型又输出了工具请求且已被后端阻止；前端仍按 `assistant_visible=true` 展示后端给出的 `assistant_message`。如果同一结果中的 `tool_calls[]` 状态全部为 `succeeded`，该消息应是“工具已成功执行但重复工具请求被阻止”的成功态说明；只有存在失败/阻断 ToolCall 时才展示为需要查看失败详情或修复输入。
